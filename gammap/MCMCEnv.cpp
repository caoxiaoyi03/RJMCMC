...
unsigned long long MCMCEnv::flagSplitTest(double &P_alloc, bool &NULLset, double &f_ui,  
	const MCMCEnv::TreeSet::value_type &split_pair, 
	const vector<pair<unsigned long, unsigned long long> > & samples, const vector<double> &ut) {
		ClusterTree &parent = getTreeFromID(split_pair.second);
		f_ui = 1.0;
		long s = 0;
		...
		if(parent.samples[split_pair.first] && split_pair.first < NumOfTP) {
				...
				for(unsigned long i = split_pair.first; i < NumOfTP; i++) {
					...
					//ut = ran_beta(2.0,2.0);
					double old_weight = parent.weights[i];
					f_ui *= (old_weight > SMALLNUM)? betad(ut[i], 2.0, 2.0): 1.0;
					/* This is a good choice for resetting the cluster indicators and weight parameter */
					parent.weights[i] = ut[i] * old_weight;
					child.weights[i] = (1 - ut[i]) * old_weight;
					f_ui *= 1.0;
					/* This is a good choice for resetting the cluster indicators and weight parameter */

				}
				for(vector<pair<unsigned long, unsigned long long> >::const_iterator itor = samples.begin();
					itor != samples.end(); itor++) {
						if(Flags(itor->first, itor->second) == parent.ID) {
							Flags(itor->first, itor->second) = child.ID;
							(parent.samples[itor->first])--;
							(child.samples[itor->first])++;
						}
				}
				NULLset = samples.size();
				for(unsigned long i = split_pair.first; i < NumOfTP; i++) {
					P_alloc *= pow(ut[i], parent.getSampleNum(i)) 
						* pow((1 - ut[i]), child.getSampleNum(i));
				}
				/*std::cout<<"new"<<std::endl;
				std::cout<<one<<std::endl;
				std::cout<<two<<std::endl;
	...
	for(TreeContainer::const_iterator itor = MapClusterTrees.begin();
		itor != MapClusterTrees.end(); itor++) {
			...
			if(cclusternum){
				...
				std::cout<<exp(K1)<<std::endl;
				std::cout<<"S1.determinant: "<<std::endl;
				std::cout<<S1.determinant()<<std::endl;
				std::cout<<"Gamma0.determinant: "<<std::endl;
				std::cout<<pow(constSigma1, (double) taonum)<<std::endl;
